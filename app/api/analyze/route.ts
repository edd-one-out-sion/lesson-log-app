...
export async function POST(req: Request) {
  try {
    ...
    // API 키가 없으면 바로 에러를 뱉게 설정
    if (!process.env.GEMINI_API_KEY) {
      return NextResponse.json({ suggestion: "서버에 API 키가 설정되지 않았습니다." }, { status: 500 });
    }
    ...
    // 에러가 나면 터미널(로그)에 구체적인 이유를 찍습니다.
    console.error("AI 분석 에러 발생:", error);
